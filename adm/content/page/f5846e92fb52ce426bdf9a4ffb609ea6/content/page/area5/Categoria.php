...
$nunitemscategoria = mysqli_query($conecta,"SELECT id FROM artigo WHERE id_cat = '$id' ");
if (!$nunitemscategoria) {
echo 'Could not run query: ' . mysqli_error();
exit;
}

$nunitemscategoriaativo = mysqli_query($conecta,"SELECT id FROM artigo WHERE id_cat = '$id'  AND ativo = 's'");
if (!$nunitemscategoriaativo) {
echo 'Could not run query: ' . mysqli_error();
exit;
}

$nunitemscategoriabloq = mysqli_query($conecta,"SELECT id FROM artigo WHERE id_cat = '$id'  AND ativo = 'n'");
...
echo"
        ...
        <td><a class='btn btn-success btn-xs' href='home.php?link=44&id=$id&nome=$nome'> Entrar </a></td>
       </tr>
    </tbody>";
	}
}
